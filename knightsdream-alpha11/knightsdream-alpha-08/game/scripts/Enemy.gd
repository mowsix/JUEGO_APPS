...
func _ready():
	...
	# Conectar la señal del HurtArea
#	if hurt_area:
...
func _physics_process(delta):
	...
	match current_state:
		State.WALK:
			_handle_walk()
		State.ATTACK:
			velocity.x = direction * move_speed * 0.3
		State.HURT:
			velocity.x = 0
		State.DEAD:
			velocity.x = 0
	...
	# ✅ CAMBIO 1: DETECTAR PAREDES Y CAMBIAR DIRECCIÓN
	if is_on_wall():
		direction *= -1
...
func _on_hurt_area_body_entered(body):
	...
	if body.has_method("take_damage"):
		...
		# Volver a caminar después de atacar
